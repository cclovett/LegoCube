        ...
        @synchronized(self)
        {
            if (!_mPersistentStoreCoordinator)
            {
                                                                        ...
                                                                        error:&error])
                {
                    ...
                    {
                        //删除DB文件
                        //file:///var/mobile/Containers/Data/Application/98D37862-1416-433B-AB37-69A5C4D82257/Documents/PGSDK/pg_edit_sdk.db
                        NSError *removeError = nil;
                        NSString *sbURLstring = dbURL.absoluteString;
                        if ([[NSFileManager defaultManager] removeItemAtURL:dbURL error:&removeError])
                        {
                            NSURL *dbURLshm = [NSURL URLWithString:[sbURLstring stringByAppendingString:@"-shm"]];
                            if ([[NSFileManager defaultManager] removeItemAtURL:dbURLshm error:&removeError])
                            {
                                NSURL *dbURLwal = [NSURL URLWithString:[sbURLstring stringByAppendingString:@"-wal"]];
                                if (![[NSFileManager defaultManager] removeItemAtURL:dbURLwal error:&removeError]) {
                                    
                                    NSLog(@"Error: remove %@",removeError);
                                }
                            }
                            else {
                                NSLog(@"Error: remove %@",removeError);
                    ...
                    if (![_mPersistentStoreCoordinator addPersistentStoreWithType:NSSQLiteStoreType
                        ...
                        NSLog(@"Error: 重新删除数据库后，数据库还是错误,请删掉App重装");
                        abort();
                    }
#else
                    NSLog(@"Error: adding persistent store. %@, %@", error, error.userInfo);
